...
announcements.post("/", asyncRoute(async (req, res) => res.status(201).json(await db.announcement.create({ data: parse(announcementSchema, req.body) }))));
for (const method of ["patch", "put"] as const) announcements[method]("/:id", asyncRoute(async (req, res) => {
...
announcements.delete("/:id", asyncRoute(async (req, res) => res.json(await db.announcement.delete({ where: { id: String(req.params.id) } }))));
