...
schedules.post("/", asyncRoute(async (req, res) => res.status(201).json(await db.schedule.create({ data: parse(scheduleSchema, req.body) }))));
schedules.patch("/:id", asyncRoute(async (req, res) => res.json(await db.schedule.update({ where: { id: String(req.params.id) }, data: parse(scheduleSchema.omit({ id: true }).partial(), req.body) }))));
